...
class Parameters:
    ...
    def __init__(self):
        ...
        self.plots = 0  # print the plots (0 -> no, 1 -> yes)
        ...
        self.R0 = self.height+self.c/2/self.PRF
        ...
        if self.waveform_sequence == 1:
            ...
            self.t_i = 10*np.arange(2 * self.cr) * (np.arange(2 * self.cr) + 1) / (2 * self.chirp_bandwidth) - \
                       self.chirp_duration * (np.floor(
                (10*np.arange(2 * self.cr) * (np.arange(2 * self.cr) + 1) + self.cr) / (2 * self.cr)))
        ...
        self.n_rg = int(2. ** np.ceil(np.log(2. * self.chirp_duration * self.rg_f_s) / np.log(2.)))
