...
folder_name = 'Up_down_Chirp_with_RCM_CR_' + str(CR) + '_n_az_2_' + str(int(np.log2(n_az))) + '_' + dt_string
...
for k in range(1, n_az-1):
  ...
  u1[:, k] = ((np.abs(t_rg - 2. * R[k] / c0) <= (tau / 2.)) *
    np.exp((-1)**k*j * np.pi * B / tau * (t_rg - 2. * R[k] / c0 - t_i_local - tau * np.floor((t_rg - 2. * R[k] / c0 + tau / 2 - t_i_local) / tau)) ** 2.)*  # chirp modulation (different STSO waveforms for different pulses)
  ...
  # NADIR
  t_i_local = 0 #t_i[(k+1) % len(t_i)]

  u2[:, k] = ((np.abs(t_rg - 2. * R_nad[k] / c0) <= (tau / 2.)) *
    np.exp((-1)**(k+1)*j * np.pi * B / tau * (t_rg - 2. * R_nad[k] / c0 - t_i_local - tau * np.floor((t_rg - 2. * R_nad[k] / c0 + tau / 2 - t_i_local) / tau)) ** 2.)*  # chirp modulation (different STSO waveforms for different pulses)
...
if (plots):
    ...
    plt.savefig(folder_name + '\Raw_data_u.pdf',bbox_inches="tight",pad_inches=0)
    plt.show()
    ...
    plt.ylabel('Range samples')
    plt.xlabel('Azimuth samples')
    plt.savefig(folder_name + '\Raw_data_u1.pdf',bbox_inches="tight",pad_inches=0)
    plt.show()
    ...
    plt.ylabel('Range samples')
    plt.xlabel('Azimuth samples')
    plt.savefig(folder_name + '\Raw_data_u2.pdf',bbox_inches="tight",pad_inches=0)
    plt.show()
...
if (plots):
    ...
    plt.ylabel('Range samples')
    plt.xlabel('Azimuth samples')
    plt.savefig(folder_name + '\Range_compressed_data_u_to_the_signal_without_nadir_removal.pdf',bbox_inches="tight",pad_inches=0)
    plt.show()
    ...
    plt.ylabel('Range samples')
    plt.xlabel('Azimuth samples')
    plt.savefig(folder_name + '\Range_compressed_data_u1_to_the_signal_without_nadir_removal.pdf',bbox_inches="tight",pad_inches=0)
    plt.show()
    ...
    plt.ylabel('Range samples')
    plt.xlabel('Azimuth samples')
    plt.savefig(folder_name + '\Range_compressed_data_u2_to_the_signal_without_nadir_removal.pdf',bbox_inches="tight",pad_inches=0)
    plt.show()
...
for k in range(1, n_az-1): u[:, k] = u[:, k] * np.exp(np.zeros(n_rg) + j * 4. * np.pi * R0 / c0 * (np.sqrt((f0 + f_rg) ** 2. - (c0 * f_az[k] / (2 * v_r)) ** 2.) - f_rg))
for k in range(1, n_az-1): u1[:, k] = u1[:, k] * np.exp(np.zeros(n_rg) + j * 4. * np.pi * R0 / c0 * (np.sqrt((f0 + f_rg) ** 2. - (c0 * f_az[k] / (2 * v_r)) ** 2.) - f_rg))
for k in range(1, n_az-1): u2[:, k] = u2[:, k] * np.exp(np.zeros(n_rg) + j * 4. * np.pi * R0 / c0 * (np.sqrt((f0 + f_rg) ** 2. - (c0 * f_az[k] / (2 * v_r)) ** 2.) - f_rg))
...
if (plots):
    ...
    plt.ylabel('Range samples')
    plt.xlabel('Azimuth samples')
    plt.savefig(folder_name + '\Focused_data_u_to_the_signal_without_dual_focusing.pdf',bbox_inches="tight",pad_inches=0)
    plt.show()
    ...
    plt.ylabel('Range samples')
    plt.xlabel('Azimuth samples')
    plt.savefig(folder_name + '\Focused_data_u1_to_the_signal_without_dual_focusing.pdf',bbox_inches="tight",pad_inches=0)
    plt.show()
    ...
    plt.ylabel('Range samples')
    plt.xlabel('Azimuth samples')
    plt.savefig(folder_name + '\Focused_data_u2_to_the_signal_without_dual_focusing.pdf',bbox_inches="tight",pad_inches=0)
    plt.show()
...
for k in range(1, n_az-1):
    ...
    h_rg = np.roll(np.less_equal(np.abs(t_rg-2*R0/c0), tau/2) *np.exp((-1)**(k+1)*j * np.pi * B / tau * (t_rg - 2. * R0 / c0 - t_i_local - tau * np.floor((t_rg - 2. * R0 / c0 + tau / 2 - t_i_local) / tau)) ** 2.), int(n_rg / 2.))
...
for k in range(1, n_az-1): u[:, k] = u[:, k] * np.exp(np.zeros(n_rg, dtype= complex) + j * 4. * np.pi * h / c0 * (np.sqrt((f0 + f_rg) ** 2. - (c0 * f_az[k] / (2 * v_r)) ** 2.) - f_rg))
for k in range(1, n_az-1): u1[:, k] = u1[:, k] * np.exp(np.zeros(n_rg, dtype= complex) + j * 4. * np.pi * h / c0 * (np.sqrt((f0 + f_rg) ** 2. - (c0 * f_az[k] / (2 * v_r)) ** 2.) - f_rg))
for k in range(1, n_az-1): u2[:, k] = u2[:, k] * np.exp(np.zeros(n_rg, dtype= complex) + j * 4. * np.pi * h / c0 * (np.sqrt((f0 + f_rg) ** 2. - (c0 * f_az[k] / (2 * v_r)) ** 2.) - f_rg))
...
# for k = 0l, n_rg - 1 do u[k, *] = u[k, *] * az_window_F
...
# data_values.write('Nadir echo removal\n\n')
# data_values.write('u[2047:2050, :] = 0.\n')
# data_values.write('u1[2047:2050, :] = 0.\n')
# data_values.write('u2[2047:2050, :] = 0.\n\n\n')
#
# u[2047:2050, :] = 0.
# u1[2047:2050, :] = 0.
# u2[2047:2050, :] = 0.
...
if (data):
    ...
    # print( 'Peak = ', 20.*np.log10(abs(u1).max()), ' dB')
    # print( 'Peak = ', 20.*np.log10(abs(u2).max()), ' dB')
    # print( 'Peak = ', 20.*np.log10(abs(u).max()), ' dB')
    #
    # print( 'Energy = ', 10.*np.log10((abs(u1)**2.).sum()), ' dB')
    # print( 'Energy = ', 10.*np.log10((abs(u2)**2.).sum()), ' dB')
    # print( 'Energy = ', 10.*np.log10((abs(u)**2.).sum()), ' dB')
...
for k in range(1, n_az-1): u[:, k] = u[:, k] * np.exp(np.zeros(n_rg, dtype= complex) + j * 4. * np.pi * R0 / c0 * (np.sqrt((f0 + f_rg) ** 2. - (c0 * f_az[k] / (2 * v_r)) ** 2.) - f_rg))
for k in range(1, n_az-1): u1[:, k] = u1[:, k] * np.exp(np.zeros(n_rg, dtype= complex) + j * 4. * np.pi * R0 / c0 * (np.sqrt((f0 + f_rg) ** 2. - (c0 * f_az[k] / (2 * v_r)) ** 2.) - f_rg))
for k in range(1, n_az-1): u2[:, k] = u2[:, k] * np.exp(np.zeros(n_rg, dtype= complex) + j * 4. * np.pi * R0 / c0 * (np.sqrt((f0 + f_rg) ** 2. - (c0 * f_az[k] / (2 * v_r)) ** 2.) - f_rg))
