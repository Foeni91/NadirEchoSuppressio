...
def antenna_pattern_signal(param):
    ...
    az_mod_sign = np.roll(np.sinc(param.antenna_length / param.wavelength * param.sin_psi) ** 2 * np.exp(
        -param.j * 4 * math.pi / param.wavelength * param.R), int(param.n_az / 2))
