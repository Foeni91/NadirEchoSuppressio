...
class Parameters:
    ...
    def __init__(self):
        ...
        self.chirp_duration = 101*10**-6#self.cr / self.chirp_bandwidth


        # Chirp compression ratio: time - bandwidth product
        self.cr = int(self.chirp_bandwidth*self.chirp_duration)
        ...
        if self.waveform_sequence == 1:
            ...
            self.t_i = 20*np.arange(2 * self.cr) * (np.arange(2 * self.cr) + 1) / (2 * self.chirp_bandwidth) - \
                       self.chirp_duration * (np.floor(
                (20*np.arange(2 * self.cr) * (np.arange(2 * self.cr) + 1) + self.cr) / (2 * self.cr)))
